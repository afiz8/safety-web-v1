...
import SafetyMoments from './pages/SafetyMoments';
...
const App = () => {
  ...
  // ========== TAMBAHAN: state token untuk JWT ==========
  ...
  // ========== TAMBAHAN: Auto login dari token yang tersimpan ==========
  useEffect(() => {
    const checkSavedToken = async () => {
      const savedToken = localStorage.getItem('token');
      if (savedToken) {
        try {
          // Verifikasi token ke backend (opsional, bisa juga langsung decode)
  ...
  // ==================== AUTH FUNCTIONS (DIUBAH UNTUK JWT) ====================
  const login = async (username, password) => {
    ...
    try {
      ...
      setLoginError('Tidak dapat terhubung ke server. Periksa koneksi backend.');
      return false;
    }
  };

  // ========== PERBAIKAN LOGOUT: panggil endpoint backend ==========
  const logout = async () => {
    // Kirim request logout ke backend jika ada token
    if (token) {
      try {
  ...
  const contextValue = {
    ...
    loginError, jsaSubmissions, API_BASE, isLoading, token
  };

  // ==================== SIDEBAR (tidak diubah, kecuali tombol logout sudah memanggil fungsi baru) ====================
  const Sidebar = () => {
    ...
    return (
      <div className={`bg-gradient-to-b from-gray-900 to-gray-800 text-white min-h-screen transition-all duration-300 fixed left-0 top-0 z-40 flex flex-col ${isCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        {/* Tombol LOGOUT - sudah menggunakan fungsi logout dari context */}
        <div className="p-3 border-t border-gray-700 mt-auto">
          <button
            ...
            className={`flex items-center w-full p-3 rounded-xl transition-all hover:bg-red-600/20 text-red-400 hover:text-red-300 ${
  ...
  const Routes = () => {
    const { currentPage, session, canAccess, isLoading } = useContext(UserContext);
    ...
    if (session.loggedIn && (currentPage === '/' || currentPage === '/login')) {
      return <DashboardHSSE />;
    }
    
    if (!session.loggedIn && currentPage === '/') {
      return <LandingPageElefante />;
    }
    ...
    if (!canAccess(currentPage)) {
      return (
        <div className="p-8 text-center">
          <h2 className="text-2xl font-bold text-red-500 mb-4">Akses Ditolak</h2>
    ...
    return pageMap[currentPage] || <DashboardHSSE />;
  };
